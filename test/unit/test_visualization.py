...
class TestVisualization(unittest.TestCase):
    ...
    def test_write_classification_report(self):
        ...
        write_classification_report(cr, _get_relative_path(__file__, '.'), 'word2vec')
        self.assertTrue(os.path.exists(_get_relative_path(__file__, cr_out_path)))
        with open(_get_relative_path(__file__, cr_out_path), 'r') as f:
            self.assertTrue(len(f.readlines()) > 0)
        os.remove(_get_relative_path(__file__, cr_out_path))

    def test_plot_confusion_matrix(self):
        cm_out_path = "word2vec.png"
        plot_confusion_matrix(self._y_pred, self._y_true, [0, 1], ['zero', 'one'], _get_relative_path(__file__, '.'), 'word2vec')
