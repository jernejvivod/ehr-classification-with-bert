...
def task_evaluate(parsed_args: dict):
    word_to_embedding = get_word_to_embedding(parsed_args['embeddings_path'])
    if parsed_args['method'] == EntityEmbeddingMethod.STARSPACE.value:
        # STARSPACE
        logger.info('Evaluating Starspace method')
        clf = get_clf_starspace(word_to_embedding)
    elif parsed_args['method'] == EntityEmbeddingMethod.WORD2VEC.value or parsed_args['method'] == EntityEmbeddingMethod.FASTTEXT.value:
        # WORD2VEC or FASTTEXT
        logger.info('Evaluating {0} method'.format('Word2Vec' if parsed_args['method'] == EntityEmbeddingMethod.WORD2VEC.value else 'fastText'))
